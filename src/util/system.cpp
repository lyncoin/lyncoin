...
const char * const BITCOIN_CONF_FILENAME = "lyncoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //lyncoin-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Lyncoin
    // macOS: ~/Library/Application Support/Lyncoin
    // Unix-like: ~/.lyncoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Lyncoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Lyncoin";
#else
    // Unix-like
    return pathRet / ".lyncoin";
#endif
#endif
}
