...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " lyncoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  lyncoin-tx [options] <hex-tx> [commands]  Update hex-encoded lyncoin transaction\n"
                "or:     lyncoin-tx [options] -create [commands]   Create hex-encoded lyncoin transaction\n"
                "\n";
